...
export function buildMockQuote(options) {
  ...
  const profile = PLATFORM_PROFILE[platformId] ?? PLATFORM_PROFILE.jd;
  const seed = `${platformId}:${itemId || keyword || 'default'}`;
  const rng = createRng(seed);
  const archetype = pickArchetype(seed, keyword, title);

  // 让「哪家最便宜」随商品变化，避免演示时永远是同一家最低
  const winnerPlatform =
    WINNER_ORDER[hashString(`winner:${itemId || keyword || 'default'}`) % WINNER_ORDER.length];
  const isWinner = winnerPlatform === platformId;

  const base = basePrice > 0 ? basePrice : archetype.base;
  const price = Math.max(
    100,
    roundYuan(base * profile.multiplier * (isWinner ? 0.96 : 1.02) * (0.985 + rng() * 0.03)),
  );

 ...
 *
 * **按天折叠**：同一自然日可能有多次观测（用户反复点比价），
 * 直接全部返回会让曲线在一天内出现密集锯齿、横轴日期重复。
 * 这里每天只保留最后一次观测（当日收盘价语义），既让曲线可读，
 * 也让「已记录 N 天」的语义与横轴刻度一致。
 *
 * @param {string} platformId
 * @param {string} itemId
...
export async function getSeries(platformId, itemId) {
  ...
  /** 按日期收口：同日多次观测只留最后一次（t 最大）。 */
  const byDate = new Map();
  for (const point of entry.points) {
    if (!point || !point.date) continue;
    const price = Number(point.finalPrice ?? point.price);
    if (!(price > 0)) continue;

    const existing = byDate.get(point.date);
    if (!existing || String(point.t ?? '') >= String(existing.t ?? '')) {
      byDate.set(point.date, { t: point.t, price: Math.round(price) });
    }
  }

  return [...byDate.entries()]
    .sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0))
    .map(([date, value]) => ({ date, price: value.price }));
}

...
/**
 * 把一个可能是任何脏值的金额安全归一化成 **非负整数分**。
 *
 * 为什么需要：上游返回体字段缺失或类型异常时会给出 null / NaN / Infinity / 负数字符串，
 * 直接 `Math.round()` 会把 NaN 传染到 finalPrice，前端就会显示「¥NaN」。
 * 这里统一兜底为 0，保证全链路金额永远是有限非负整数。
 *
 * @param {unknown} value
 * @returns {number} 非负整数分；非法输入返回 0
 */
...
export function buildQuote(input) {
  const price = normalizeCents(input.price);
  const coupons = (input.coupons ?? [])
    .filter((coupon) => coupon && typeof coupon === 'object')
    .map((coupon, index) => ({
      id: coupon.id ?? `${input.platformId}-coupon-${index}`,
      name: coupon.name ?? '优惠券',
      amount: normalizeCents(coupon.amount),
      threshold: normalizeCents(coupon.threshold),
      expireText: coupon.expireText ?? '',
      source: coupon.source ?? input.platformName,
      // 券的真实领取链接（联盟转链后返回）；空串 = 拿不到，前端不显示「立即领券」
      couponUrl: typeof coupon.couponUrl === 'string' ? coupon.couponUrl : '',
      best: false,
    }))

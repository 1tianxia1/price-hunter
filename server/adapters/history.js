...
export async function queryHistory(options = {}) {
  ...
  if (dataoke.hasKeys() && dataoke.supports(platformId) && itemId) {
    try {
      ...
      if (normalized) {
        // 大淘客同一日期可能有多个价格点（日内多次上架价变动），
        // 展示「已记录 N 天」必须按**去重日期**算，用 points.length 会把一天的多个点算成多天
        const recordedDays = new Set(normalized.points.map((point) => point.date)).size;
        return { ...normalized, source: SOURCE_DATAOKE, recordedDays };
      }
    ...
    try {
      ...
      if (normalized) {
        return {
          ...
          recordedDays: new Set(normalized.points.map((point) => point.date)).size,
        };
      }
